...
async function verifyPlatformFolders(platforms, project) {
  ...
  if (platforms.indexOf('ios') >= 0 && !(await project.iosExists())) {
    ...
    log_1.logger.warn(
      `iOS platform not found at ${
        ((_a = project.config.ios) === null || _a === void 0 ? void 0 : _a.path) || ''
      }, skipping iOS generation`
    );
  }
  if (platforms.indexOf('android') >= 0 && !(await project.androidExists())) {
    platforms.splice(platforms.indexOf('android'), 1);
    log_1.logger.warn(
      `Android platform not found at ${
        ((_b = project.config.android) === null || _b === void 0 ? void 0 : _b.path) || ''
      }, skipping android generation`
    );
  }
}
